...
static int cluster_init(const struct cpumask *mask)
{
	...
	/* Background-Max: keep reclaim/swap responsive by avoiding overly aggressive core offlining. */
	if (first_cpu == 0 && cluster->num_cpus >= 2)
		cluster->min_cpus = 2;
	else
		cluster->min_cpus = 1;
	cluster->max_cpus = cluster->num_cpus;
	cluster->need_cpus = cluster->num_cpus;
	/* Background-Max: reduce core thrash (online/offline toggling). */
	cluster->offline_delay_ms = 1000;
