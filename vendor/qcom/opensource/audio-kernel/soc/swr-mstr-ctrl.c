				...
				int core_type, bool enable)
{
			...
			if (enable) {
				if (!swrm->dev_up) {
					dev_dbg(swrm->dev, "%s: device is down or SSR state\n",
							__func__);
					pr_debug("%s: device is down or SSR state\n",
							__func__);
	...
	pr_debug("%s: hw_clk_en: %d audio_core_clk_en: %d\n",
		__func__, swrm->hw_core_clk_en, swrm->aud_core_clk_en);
	return ret;
}
...
handle_irq:
	...
	if (intr_sts_masked && !pm_runtime_suspended(swrm->dev)) {
		dev_dbg(swrm->dev, "%s: new interrupt received 0x%x\n",
			__func__, intr_sts_masked);
		pr_debug("%s: new interrupt received 0x%x\n", __func__,
				intr_sts_masked);
		goto handle_irq;
	}
...
static int swrm_runtime_resume(struct device *dev)
{
	...
	dev_dbg(dev, "%s: pm_runtime: resume, state:%d\n",
		__func__, swrm->state);
	pr_debug("%s: pm_runtime: resume, state:%d\n",
		__func__, swrm->state);
...
static int swrm_runtime_suspend(struct device *dev)
{
	...
	pr_debug("%s: pm_runtime: suspend state: %d\n",
		__func__, swrm->state);
...
exit:
	...
	mutex_unlock(&swrm->reslock);
	mutex_unlock(&swrm->runtime_lock);
	pr_debug("%s: pm_runtime: suspend done state: %d\n",
		__func__, swrm->state);
...
int swrm_wcd_notify(struct platform_device *pdev, u32 id, void *data)
{
	...
	case SWR_DEVICE_DOWN:
		...
		pr_debug("%s: swr master down called\n", __func__);
		mutex_lock(&swrm->mlock);
		if (swrm->state == SWR_MSTR_DOWN)
			dev_dbg(swrm->dev, "%s:SWR master is already Down:%d\n",
				__func__, swrm->state);
	...
	case SWR_DEVICE_UP:
		...
		pr_debug("%s: swr master up called\n", __func__);
		mutex_lock(&swrm->devlock);
		if (!swrm->dev_up) {
