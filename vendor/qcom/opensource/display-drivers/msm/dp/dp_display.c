...
#define DRM_DP_IPC_NUM_PAGES 4

/*
 * Background-Max: relax overly aggressive resume-latency QoS votes from DP.
 * Smaller latency values are stricter; clamping raises them to reduce always-on power draw.
 * 0 disables clamping.
 */
static uint bg_max_dp_min_resume_latency_us = 2000;
module_param(bg_max_dp_min_resume_latency_us, uint, 0644);
MODULE_PARM_DESC(bg_max_dp_min_resume_latency_us,
		 "Minimum DEV_PM_QOS_RESUME_LATENCY (us) enforced by DP (0 disables).");

static inline u32 bg_max_clamp_dp_latency(u32 latency)
{
	if (!bg_max_dp_min_resume_latency_us)
		return latency;
	if (latency < bg_max_dp_min_resume_latency_us)
		return bg_max_dp_min_resume_latency_us;
	return latency;
}
...
static void dp_display_qos_request(struct dp_display_private *dp, bool add_vote)
{
	...
	latency = dp->parser->qos_cpu_latency;
	latency = bg_max_clamp_dp_latency(latency);
	mask = dp->parser->qos_cpu_mask;
