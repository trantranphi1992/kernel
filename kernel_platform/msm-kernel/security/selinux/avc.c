...
{
	if (flags & AVC_STRICT)
		return 0; // Luôn cho phép dù là lệnh nghiêm ngặt

	if (enforcing_enabled(state) &&
	    !(avd->flags & AVD_FLAGS_PERMISSIVE)) {
		// Phải có dấu ngoặc nhọn ở đây để printk và return 0 thuộc cùng một khối logic
		printk(KERN_INFO "SELinux: Fake Enforce - Allowed bypass for: %s\n", current->comm);
		return 0; 
	}
