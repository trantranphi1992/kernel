...
/*
 * Background-Max: relax overly aggressive PM QoS requests from WLAN.
 * These QoS requests can keep CPUs from entering deeper idle states and
 * increase always-on power draw (which indirectly makes background app
 * retention worse via thermal/power policies).
 *
 * Values are in microseconds. 0 disables clamping.
 */
static uint bg_max_wlan_min_resume_latency_us = 2000;
module_param(bg_max_wlan_min_resume_latency_us, uint, 0644);
MODULE_PARM_DESC(bg_max_wlan_min_resume_latency_us,
		 "Minimum DEV_PM_QOS_RESUME_LATENCY (us) enforced by WLAN (0 disables).");

static uint bg_max_wlan_min_cpu_dma_latency_us = 2000;
module_param(bg_max_wlan_min_cpu_dma_latency_us, uint, 0644);
MODULE_PARM_DESC(bg_max_wlan_min_cpu_dma_latency_us,
		 "Minimum PM_QOS_CPU_DMA_LATENCY (us) enforced by WLAN (0 disables).");

static inline unsigned int bg_max_clamp_latency(unsigned int latency,
					      ...
					      unsigned int latency)
{
	...
	default_latency = bg_max_clamp_latency(default_latency,
					 bg_max_wlan_min_resume_latency_us);
					     ...
					     cpumask_t *pm_qos_cpu_mask)
{
	...
	if (cpumask_empty(pm_qos_cpu_mask))
		...
		pm_qos_update_request(&hdd_ctx->pm_qos_req,
			 bg_max_clamp_latency(1,
					 bg_max_wlan_min_cpu_dma_latency_us));
}
