...
/* dget_parent() is not exported on some kernels -> causes modpost undefined symbol.
 * Provide a local helper that grabs a ref on dentry->d_parent safely.
 */
static inline struct dentry *dhd_dget_parent(struct dentry *dentry)
{
	struct dentry *parent;
	/* d_parent is protected by d_lock */
...
dhd_file_delete(char *path)
{
		...
		FALSE)
	{
		...
		dir = dhd_dget_parent(file_path.dentry);
