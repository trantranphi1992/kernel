...
/*
 * Background-Max: cap very long qdf wakelock timeout requests.
 * This helps avoid "stuck awake for minutes" cases due to overly large timeouts.
 * 0 disables capping.
 */
static uint bg_max_wakelock_timeout_cap_ms = 60000;
module_param(bg_max_wakelock_timeout_cap_ms, uint, 0644);
MODULE_PARM_DESC(bg_max_wakelock_timeout_cap_ms,
		 "Cap (max) for qdf_wake_lock_timeout_acquire timeout in ms (0 disables).");
...
{
	if (bg_max_wakelock_timeout_cap_ms && msec > bg_max_wakelock_timeout_cap_ms)
		msec = bg_max_wakelock_timeout_cap_ms;
