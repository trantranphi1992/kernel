...
/*
 * Allow modest slack on internal prediction timers so their wakeups can be
 * coalesced with other timers. These timers are advisory (governor-internal)
 * and do not need hard real-time precision.
 */
static inline u64 qcom_lpm_timer_slack_ns(u64 time_ns)
{
	/* 5% slack, clamped to [100us, 10ms] */
	u64 slack = time_ns / 20;

	if (slack < 100 * NSEC_PER_USEC)
		slack = 100 * NSEC_PER_USEC;
	if (slack > 10 * NSEC_PER_MSEC)
		slack = 10 * NSEC_PER_MSEC;
...
static void histtimer_start(uint32_t time_ns)
{
	...
	hrtimer_start_range_ns(cpu_histtimer, hist_ktime,
		qcom_lpm_timer_slack_ns((u64)time_ns * NSEC_PER_USEC),
...
static void biastimer_start(uint32_t time_ns)
{
	...
	hrtimer_start_range_ns(cpu_biastimer, bias_ktime,
		qcom_lpm_timer_slack_ns((u64)time_ns),
		HRTIMER_MODE_REL_PINNED);
}
			      ...
			      unsigned long val, void *ptr)
{
	...
	old_val = READ_ONCE(cpu_gov->last_qos_val);
	WRITE_ONCE(cpu_gov->last_qos_val, new_val);

	/*
	 * For DEV_PM_QOS_RESUME_LATENCY, smaller values mean stricter latency
	 * requirements. Only wake remote CPUs when constraints become stricter.
	 */
	if (new_val >= old_val)
		return NOTIFY_OK;
...
static int lpm_online_cpu(unsigned int cpu)
{
	...
	cpu_gov->last_qos_val = (s64)dev_pm_qos_read_value(dev,
					   DEV_PM_QOS_RESUME_LATENCY);
