...
/*
 * Background-Max: relax overly aggressive CPU resume-latency QoS votes from display.
 * Smaller latency values are stricter; clamping raises them to reduce always-on power draw.
 * 0 disables clamping.
 */
static uint bg_max_sde_min_resume_latency_us = 2000;
module_param(bg_max_sde_min_resume_latency_us, uint, 0644);
MODULE_PARM_DESC(bg_max_sde_min_resume_latency_us,
		 "Minimum DEV_PM_QOS_RESUME_LATENCY (us) enforced by SDE (0 disables).");

static inline u32 bg_max_clamp_sde_latency(u32 latency)
{
	if (!bg_max_sde_min_resume_latency_us)
		return latency;
	if (latency < bg_max_sde_min_resume_latency_us)
		return bg_max_sde_min_resume_latency_us;
	return latency;
}
...
static void _sde_encoder_pm_qos_add_request(struct drm_encoder *drm_enc)
{
	...
	cpu_dma_latency = bg_max_clamp_sde_latency(cpu_dma_latency);
