...
static unsigned long chg_ulog_delay_jiffies(u32 time_ms)
{
	unsigned long j = msecs_to_jiffies(time_ms);

	/* Round long delays to help coalesce wakeups */
	if (j >= HZ)
		j = round_jiffies(j);
...
static void chg_ulog_work(struct work_struct *work)
{
	...
	if (rc)
		...
		queue_delayed_work(system_freezable_power_efficient_wq,
				&cd->ulog_work, chg_ulog_delay_jiffies(cd->log_time_ms));
}
