...
static inline u64 qcom_hrtimer_slack_ns(ktime_t t)
{
	u64 ns = ktime_to_ns(t);
	u64 slack = ns / 20; /* 5% */

	if (slack < 100000ULL) /* 100 us */
		slack = 100000ULL;
	if (slack > 10ULL * 1000 * 1000) /* 10 ms */
		slack = 10ULL * 1000 * 1000;
	...
	if (start) {
		...
		if (llcc_priv->expires) {
			if (hrtimer_is_queued(&llcc_priv->hrtimer))
				...
				hrtimer_start_range_ns(&llcc_priv->hrtimer,
				llcc_priv->expires,
				qcom_hrtimer_slack_ns(llcc_priv->expires),
