...
/*
 * Background-Max: relax overly aggressive resume-latency QoS votes from EVA/CVP.
 * Smaller latency values are stricter; clamping raises them to reduce always-on power draw.
 * 0 disables clamping.
 */
static uint bg_max_cvp_min_resume_latency_us = 2000;
module_param(bg_max_cvp_min_resume_latency_us, uint, 0644);
MODULE_PARM_DESC(bg_max_cvp_min_resume_latency_us,
		 "Minimum DEV_PM_QOS_RESUME_LATENCY (us) enforced by CVP (0 disables).");

static inline u32 bg_max_clamp_cvp_latency(u32 latency)
{
	if (!bg_max_cvp_min_resume_latency_us)
		return latency;
	if (latency < bg_max_cvp_min_resume_latency_us)
		return bg_max_cvp_min_resume_latency_us;
	return latency;
}
...
static int iris_hfi_core_init(void *device)
{
	...
	if (dev->res->pm_qos.latency_us) {
		u32 latency_us_clamped = bg_max_clamp_cvp_latency(dev->res->pm_qos.latency_us);
		int err = 0;
		u32 i, cpu;
