...
bool sec_bat_check_dc_step_charging(struct sec_battery_info *battery)
{
	...
	/*
	 * Hold "as much as the adapter can provide" under a given SOC by dynamically calculating
	 * IOUT from VBUS (P = V * I). We cap requested power at 45W, but auto-scale down when the
	 * adapter (e.g., 25W) can't do 45W. Thermal/SIOP voters still apply.
	 */
	{
		const int hold_max_soc = 70;
		const int req_cap_mw = 45000; /* 45W upper cap */

		if (is_pd_apdo_wire_type(battery->cable_type) &&
		    battery->capacity >= 0 && battery->capacity < hold_max_soc) {
			int vbus_uv = 0, vbus_mv = 0;
			int target_iout_ma = 0;
			int max_iout_ma = battery->pdata->dc_step_chg_val_iout[age_step][0];
			int avail_power_mw = 0;
			int target_power_mw = req_cap_mw;

			/* Prefer negotiated PD max if available; fallback to other max/present power signals */
			if (battery->pd_max_charge_power > 0)
				avail_power_mw = battery->pd_max_charge_power;
			else if (battery->max_charge_power > 0)
				avail_power_mw = battery->max_charge_power;
			else if (battery->charge_power > 0)
				avail_power_mw = battery->charge_power;

			/* If we don't know available power, don't force anything (avoid mode ping-pong) */
			if (avail_power_mw <= 0)
				goto __skip_hold_power;

			if (avail_power_mw < target_power_mw)
				target_power_mw = avail_power_mw;
			...
			if (vbus_mv > 0) {
				target_iout_ma = (target_power_mw * 1000) / vbus_mv;

				if (target_iout_ma > max_iout_ma)
					target_iout_ma = max_iout_ma;
				if (target_iout_ma < 0)
					target_iout_ma = 0;

				/* Program direct charger current (often half of total output) */
				val.intval = target_iout_ma / 2;
				...
				sec_vote(battery->fcc_vote, VOTER_CABLE, true, target_iout_ma);
				sec_vote_refresh(battery->fcc_vote);

				pr_info("%s : hold %dmW (avail=%dmW) under %d%% (VBUS=%dmV, IOUT=%dmA)\n",
					__func__, target_power_mw, avail_power_mw, hold_max_soc, vbus_mv, target_iout_ma);
