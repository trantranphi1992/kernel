...
static void tethering_stats_poll_queue(struct work_struct *work)
{
	...
	if (ipa3_rmnet_ctx.polling_interval != 0)
		queue_delayed_work(system_freezable_wq, &ipa_tether_stats_poll_wakequeue_work,
			round_jiffies_relative(msecs_to_jiffies(ipa3_rmnet_ctx.polling_interval*1000)));
}
...
int rmnet_ipa3_poll_tethering_stats(struct wan_ioctl_poll_tethering_stats *data)
{
	...
	queue_delayed_work(system_freezable_wq, &ipa_tether_stats_poll_wakequeue_work, 0);
	return 0;
}
